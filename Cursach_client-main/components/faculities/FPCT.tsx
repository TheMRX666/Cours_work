...
export const FPCT = () => {
    ...
    const filteredStudents = apiData.filter((student) => student.faculty === 'FPCT' && student.group === group)
    ...
    return(
        <div className={cn(styles.faculty__section, category === 4 ? '' : "hideOf")}>
            <h2>Факультет психології та соціальних технологій</h2>
            <p className='length'>Кількість студентів: {filteredStudents.length}</p>
            {showVg === false ?
            apiData.filter((student) => student.faculty === 'FPCT' && student.group === group).map((student, index) => (
            ...
            :
            apiData.filter((student) => student.faculty === 'FPCT' && student.group === group && student.dateDeduction !== '').map((student, index) => (
                <Link href={'student/' + student.id} key={student.id} className={styles.faculty__section_student}>
                    <span className={styles.student_index}>{index + 1}.</span>
                    <div className={styles.student_box}>
                        <span className={styles.student_label}>ПІБ</span>
