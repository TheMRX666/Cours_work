...
export const FICT = () => {
    ...
    const filteredStudents = apiData.filter((student) => student.faculty === 'FICT' && student.group === group)
    ...
    return(
        <div className={cn(styles.faculty__section, category === 2 ? '' : "hideOf")}>
            <h2>Факультет інформаційних систем та технологій</h2>
            <p className='length'>Кількість студентів: {filteredStudents.length}</p>
            {showVg === false ?
            apiData.filter((student) => student.faculty === 'FICT' && student.group === group).map((student, index) => (
            ...
            :
            apiData.filter((student) => student.faculty === 'FICT' && student.group === group && student.dateDeduction !== '').map((student, index) => (
                <Link href={'student/' + student.id} key={student.id} className={styles.faculty__section_student}>
                    <span className={styles.student_index}>{index + 1}.</span>
                    <div className={styles.student_box}>
                        <span className={styles.student_label}>ПІБ</span>
